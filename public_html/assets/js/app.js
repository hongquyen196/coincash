								...
								submitHandler: function(form) {
									...
									jQuery(form).ajaxSubmit({
										...
										success: 	function(data) {
											...
											// SMTP ERROR
											if (data == '_failed_') {
												_toastr("SMTP ERROR! Please, check your config file!",_Tposition,"primary",false);
											}
											// CAPTCHA ERROR
											else if(data == '_CAPTCHA_') {
												//_toastr("Invalid Captcha!",_Tposition,"error",false);
											
												_toastr("Mã xác nhận hình ảnh không hợp lệ!","top-right","error",false);	

												//jQuery(form).find(".alert").removeClass('alert-success').addClass('alert-danger').html("Mã xác nhận hình ảnh không hợp lệ!").show();
											}
											else if(data == '_MONEY_'){

												_toastr("Số dư không đủ để thanh toán!","top-right","warning",false);		

												setInterval( function() {
													window.location.reload();
												}, 5000);			

												//jQuery(form).find(".alert").removeClass('alert-success').addClass('alert-danger').html("Có lỗi xãy ra!").show();
			
											} 

											else if(data == '_COIN_'){

												_toastr("Bạn chưa đủ điều kiện thanh toán!","top-right","warning",false);		
											...
											// SUCCESS
												...
												_toastr("Gửi yêu cầu thành công!","top-right","success",false);

												setInterval( function() {
													window.location.reload();
												}, 5000);

												//jQuery(form).find(".alert").removeClass('alert-danger').addClass('alert-success').html("Gửi yêu cầu thành công!").show();
											} 

											else {

												_toastr("Có lỗi xãy ra!","top-right","info",false);					
